...
scatter(uc_TFS_batch(idx).^1,uc_speed_batch(idx),30,'b','MarkerFaceColor','b');
hold on;
idx = find(is_wind_batch==0 & baro_force_batch==0);
scatter(uc_TFS_batch(idx).^1,uc_speed_batch(idx),30,'r','MarkerFaceColor','r');
idx = find(baro_force_batch~=0);
scatter(uc_TFS_batch(idx).^1,uc_speed_batch(idx),30,'g','MarkerFaceColor','g');
plot([0 0],[-0.02 0.08],'k--');
plot([-0.02 0.04],[0 0],'k--');
hold off
xlabel('Topographic form stress (N/m^2)');
ylabel('Undercurrent speed (m/s)');
legend('Wind forcing','Random forcing','Pressure forcing','Location','SouthEast');
set(gca,'FontSize',14);
...
idx = find(is_wind_batch==1 & baro_force_batch==0);
scatter(eddyforce_theory_coeff.*eddyforce_theory(idx),uc_eddyforce_batch(idx),30,'b','MarkerFaceColor','b');
hold on;
idx = find(is_wind_batch==0 & baro_force_batch==0);
scatter(eddyforce_theory_coeff.*eddyforce_theory(idx),uc_eddyforce_batch(idx),30,'r','MarkerFaceColor','r');
idx = find(baro_force_batch ~= 0);
scatter(eddyforce_theory_coeff.*eddyforce_theory(idx),uc_eddyforce_batch(idx),30,'g','MarkerFaceColor','g');
plot([0 0.012],[0 0.012],'k--');
hold off
...
drag_theory = rho0.*drag_coeff_batch.*uc_speed_batch.*uc_EKE_batch.^.5;
figure(60)
idx = 1:Nsims;
drag_theory_coeff = drag_theory(idx) \ (-uc_quadBotDrag_batch(idx));
idx = find(is_wind_batch==1 & baro_force_batch==0);
scatter(drag_theory_coeff.*drag_theory(idx),-uc_quadBotDrag_batch(idx),30,'b','MarkerFaceColor','b');
hold on;
idx = find(is_wind_batch==0 & baro_force_batch==0);
scatter(drag_theory_coeff.*drag_theory(idx),-uc_quadBotDrag_batch(idx),30,'r','MarkerFaceColor','r');
idx = find(baro_force_batch ~= 0);
scatter(drag_theory_coeff.*drag_theory(idx),-uc_quadBotDrag_batch(idx),30,'g','MarkerFaceColor','g');
plot([0 0.012],[0 0.012],'k--');
hold off
...
figure(64);
idx = find(is_wind_batch==1 & baro_force_batch==0);
scatter(eddyforce_theory_coeff.*eddyforce_theory(idx),drag_theory_coeff.*drag_theory(idx),30,'b','MarkerFaceColor','b');
hold on;
idx = find(is_wind_batch==0 & baro_force_batch==0);
scatter(eddyforce_theory_coeff.*eddyforce_theory(idx),drag_theory_coeff.*drag_theory(idx),30,'r','MarkerFaceColor','r');
idx = find(baro_force_batch ~= 0);
scatter(eddyforce_theory_coeff.*eddyforce_theory(idx)+uc_baroForcing_batch(idx)/3,drag_theory_coeff.*drag_theory(idx),30,'g','MarkerFaceColor','g');
plot([0 0.012],[0 0.012],'k--');
hold off
xlabel('Eddy force, theory (N/m^2)');
...
set(gca,'FontSize',14);
legend('Wind forcing','Random forcing','Pressure forcing','Location','SouthEast');
r = corr(eddyforce_theory_coeff.*eddyforce_theory+uc_baroForcing_batch/3,drag_theory);
text(1e-3,1e-2,['r^2 = ',num2str(r^2)],'FontSize',14);
...
uc_speed_theory = (eddyforce_theory_coeff.*eddyforce_theory+uc_baroForcing_batch/3)./(drag_theory_coeff.*rho0.*drag_coeff_batch.*uc_EKE_batch.^.5);
figure(65);
idx = find(is_wind_batch==1 & baro_force_batch==0);
scatter(uc_speed_theory(idx),uc_speed_batch(idx),30,'b','MarkerFaceColor','b');
hold on;
idx = find(is_wind_batch==0 & baro_force_batch==0);
scatter(uc_speed_theory(idx),uc_speed_batch(idx),30,'r','MarkerFaceColor','r');
idx = find(baro_force_batch ~= 0);
scatter(uc_speed_theory(idx),uc_speed_batch(idx),30,'g','MarkerFaceColor','g');
plot([0 0.1],[0 0.1],'k--');
hold off
...
text(1e-2,1e-1,['r^2 = ',num2str(r^2)],'FontSize',14);


figure(66);
scatter(log10(eddyforce_theory_coeff.*eddyforce_theory),log10(drag_theory_coeff.*drag_theory),20,'g')
r = corr(log10(abs(eddyforce_theory_coeff.*eddyforce_theory)),log10(abs(drag_theory_coeff.*drag_theory)));
